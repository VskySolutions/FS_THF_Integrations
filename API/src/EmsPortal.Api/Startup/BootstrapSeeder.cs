...
public static class BootstrapSeeder
{
    ...
    private static async Task SeedSystemRolesAsync(IRoleRepository roles, IUnitOfWork unitOfWork, CancellationToken cancellationToken)
    {
        var definitions = new (string Name, string Description, IReadOnlyList<string> Permissions)[]
        {
            ...
            // The four REMS seats. They grant nothing (Permissions.ForSeatRole) — holding one makes the
            ...
            // every tenant without anyone building four lists by hand first.
            ...
            (Roles.ManagingShareholder, "REMS Managing Shareholder: approves every engagement the firm routes.", Permissions.ForSeatRole()),
        };
