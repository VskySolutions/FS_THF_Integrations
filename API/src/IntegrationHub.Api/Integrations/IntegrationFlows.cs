...
public static class IntegrationFlows
{
    ...
    public static IReadOnlyDictionary<string, string> For(SystemName source, SystemName target)
        => source == SystemName.Concur && target == SystemName.Maconomy
            ? ConcurToMaconomy
    ...
    public static IReadOnlyList<FlowDescriptor> All()
        => ConcurToMaconomy.Select(kv => new FlowDescriptor(kv.Key, kv.Value, SystemName.Concur, SystemName.Maconomy)).ToList();
